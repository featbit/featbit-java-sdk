...
public abstract class Utils {
    ...
    public static <T> T parseJsonObject(String value, T defaultValue, Class<T> clazz, boolean useDefaultValue) {
        if (useDefaultValue) return defaultValue;
        try {
            return JsonHelper.deserialize(value, clazz);
        } catch (JsonParseException ex) {
            logger.error("FB JAVA SDK: unexpected error in json parsing", ex);
            return defaultValue;
        }
    }
